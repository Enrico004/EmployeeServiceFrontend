...
export class EmployeeWithSkill {
  ...
  public getEmpWithSkillId(): EmployeeWithSkillID {
    let skillId: EmployeeWithSkillID = new EmployeeWithSkillID(this.id, this.lastName, this.firstName, this.street, this.city, this.postcode, this.phone, this.skillSet?.map((item) => item.id));
    return skillId;
  }
    **/
}
